-- PASSMATE Payment P1-2/P1-3: converge verified payment events from
-- browser reconciliation and signed webhooks without double side effects.
...
begin
  ...
  if p_event_type = 'paid' then
    ...
    if v_attempt.status = 'paid' and v_order.status = 'paid' then
      if v_attempt.provider_payment_id is distinct from p_provider_payment_id then
        raise exception 'verified provider payment identity mismatch'
          using errcode = 'check_violation';
      end if;
    ...
    if v_attempt.status = 'failed' and v_order.status = 'failed' then
      return 'already_applied';
    end if;
    ...
    if v_attempt.status = 'cancelled' and v_order.status = 'cancelled' then
      return 'already_applied';
    end if;
    ...
    if v_attempt.status = 'refunded' and v_order.status = 'refunded' then
      return 'already_applied';
    end if;
