-- Preserve one entitlement grant per successful source order and product version.
-- A later purchase must never overwrite an earlier purchase grant: refund handling
-- already revokes only rows whose source_order_id matches the refunded order.

alter table public.entitlements
  drop constraint if exists entitlements_user_id_product_id_key;

alter table public.entitlements
  add constraint entitlements_user_order_product_version_key
  unique (user_id, source_order_id, product_id, product_version_id);

-- Recover grants that the former (user_id, product_id) upsert could have
-- overwritten. Existing rows win so that deliberate administrative revocations
-- are not silently reversed by this migration.
...
select
  ...
  o.id,
  case when o.status = 'paid' then 'active' else 'revoked' end,
  coalesce(o.paid_at, o.created_at),
  case
    when o.status = 'refunded' then coalesce(pa.refunded_at, o.updated_at)
    else null
  end
...
join (
  ...
  on oi.order_id = o.id
left join lateral (
  select max(a.refunded_at) as refunded_at
