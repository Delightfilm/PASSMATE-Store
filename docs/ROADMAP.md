...
|---|---:|---|
| 핵심노트 | 기본 출시가 5,900원 | 핵심개념 요약노트 + 공식·수치 한눈표 + 시험 직전 체크리스트 |
| 시험대비 완성패키지 | 기본 출시가 9,900원 | 핵심노트 전체 + 상세 개념해설서 + 단원별 확인문제·해설 |

- 별도 CRAM/벼락치기 PDF는 만들지 않는다.
- D-1/시험직전 암기는 CORE 마지막에 포함한다.
- 내부 SKU 코드는 CORE/PASS를 유지한다.
- 시험대비 완성패키지는 핵심노트 전체를 포함한다.
